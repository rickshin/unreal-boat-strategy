...
void ARTSPlayerController::OnUIButton(int32 Index)
{
	...
	// Morphers: ONE unit morphs per press, and the new cocoon leaves the
	// selection immediately, so hammering a button walks through the brood
	// larva by larva (and the panel always shows live buttons).
	for (const FEntityId E : Selection)
	{
		const FUnitC* U = G.World.Unit.Find(E);
		...
		const FName Target = T->MorphInto[Index];
		const FUnitTpl* TargetTpl = G.Content.Unit(G.Players[Me].FactionId, Target);
		if (!TargetTpl || !G.CanAfford(Me, TargetTpl->Cost)) { return; }   // keep it selected
		...
		Selection.Remove(E);
		if (AUnitActor* Actor = Mode->FindEntityActor(E)) { Actor->SetSelected(false); }
		return;
	}
}
